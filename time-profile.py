...
@click.option('--highest-hit-contributors', type=int, default=10, help='Plot truth data from the number of tracks that contribute to the most number of hits')
def main(input_data, output_pdf, hit_threshold, highest_hit_contributors):
    
    if hit_threshold is not None and highest_hit_contributors is not None:
        print('You can only specify one of --hit-threshold or --highest-hit-contributors')
        exit(1)
    ...
    hit_track = ak.to_numpy(hit_data['h_track_id'])
    ...
    def add_truth_particle(ax, hit_track, part_x, part_y, part_id, part_pdg):
        all_hit_track_ids = np.unique(hit_track)

        how_many_hits_per_track_id = {}
        for hit_track_id in all_hit_track_ids:
            how_many_hits_per_track_id[hit_track_id] = np.countnotzeros(hit_track==hit_track_id)
        ...
        if highest_hit_contributors is not None:
            ...
            if highest_hit_contributors >= len(number_of_hits):
                ...
                hit_threshold = number_of_hits[highest_hit_contributors+1]-1
        ...
        for track_id, nhit in how_many_hits_per_track_id.items():
            ...
            xs = x_part[mask]
            ys = y_part[mask]
            ax.plot(xs, ys, label=part_pdg[0])
    ...
    with PdfPages(output_pdf) as pdf:
        ...
        add_truth_particle(ax0, hit_id_yz, truth_z, truth_x, truth_id, truth_pdg)
        ax0.set_xlabel('z [mm]')
        ax0.set_ylabel('x [mm]')
        ...
        add_truth_particle(ax1, hit_id_xz, truth_z, truth_y, truth_id, truth_pdg)
        ax1.set_xlabel('z [mm]')
        ax1.set_ylabel('y [mm]')
